...
import type { Tour, TourStop, Address, Transport, TripTemplate, ReportTemplate } from '@/types';
import { tourData, customerData, fleetData, trailerData, addressData, dieselpreiseData, transportData, tripTemplateData, reportTemplateData } from '@/lib/data';
...
const FahrbefehlDialog = ({ tour, onGenerate }: { tour: Tour, onGenerate: (html: string) => void }) => {
    
    useEffect(() => {
        const fahrbefehlTemplate = reportTemplateData.find(t => t.id === 'tpl-fahrbefehl');
        if (!fahrbefehlTemplate) {
            onGenerate('<p>Fahrbefehl-Vorlage nicht gefunden.</p>');
            return;
        }

        const tourDetails = {
            vehicle: fleetData.find(v => v.id === tour.vehicleId),
            trailer: trailerData.find(t => t.id === tour.trailerId),
            driverName: tour.driverId?.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())
        };

        let processedHtml = fahrbefehlTemplate.content;

        const replacements: { [key: string]: string | undefined } = {
            'tour.tourNumber': tour.tourNumber,
            'tour.tourDate': formatDate(tour.tourDate, 'dd.MM.yyyy'),
            'tour.driverName': tourDetails.driverName,
            'tour.vehicleKennzeichen': tourDetails.vehicle?.kennzeichen,
            'tour.vehicleInfo': `${tourDetails.vehicle?.hersteller} ${tourDetails.vehicle?.modell}`,
            'tour.trailerKennzeichen': tourDetails.trailer?.kennzeichen,
            'tour.trailerInfo': `${tourDetails.trailer?.hersteller}`,
            'tour.customerReference': tour.customerReference,
        };

        for (const [key, value] of Object.entries(replacements)) {
            processedHtml = processedHtml.replace(new RegExp(`{{${key}}}`, 'g'), value || 'N/A');
        }
        ...
        processedHtml = processedHtml.replace('{{#each tour.stops}}...{{/each}}', stopsHtml);

        onGenerate(processedHtml);
...
const ReportDisplayDialog = ({ htmlContent, onPrint, isOpen, onOpenChange, title }: { htmlContent: string; onPrint: () => void; isOpen: boolean; onOpenChange: (open: boolean) => void, title: string }) => {
    return (
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-4xl">
                ...
                <div dangerouslySetInnerHTML={{ __html: htmlContent }} className="py-4 max-h-[70vh] overflow-y-auto pr-4" />
                <DialogFooter>
                    <Button variant="secondary" onClick={onPrint}>Drucken</Button>
                    <DialogClose asChild>
...
function TourReportPageInternal() {
    ...
    const [selectedTourForFahrbefehl, setSelectedTourForFahrbefehl] = useState<Tour | null>(null);
    const [fahrbefehlHtml, setFahrbefehlHtml] = useState<string>('');
    const [isFahrbefehlOpen, setIsFahrbefehlOpen] = useState(false);
    const printRef = useRef<HTMLDivElement>(null);
    ...
    const handleOpenFahrbefehl = (tour: Tour) => {
        setSelectedTourForFahrbefehl(tour);
        setIsFahrbefehlOpen(true);
    };

    const handlePrintFahrbefehl = () => {
        const printWindow = window.open('', '', 'height=800,width=800');
        if (printWindow) {
           const fahrbefehlTemplate = reportTemplateData.find(t => t.id === 'tpl-fahrbefehl');
           const styles = fahrbefehlTemplate?.content.match(/<style>([\s\S]*?)<\/style>/)?.[1] || '';

           printWindow.document.write(`<html><head><title>Fahrbefehl</title><style>${styles}</style></head><body>`);
           printWindow.document.write(fahrbefehlHtml);
           printWindow.document.write('</body></html>');
           printWindow.document.close();
           printWindow.focus();
           printWindow.print();
    ...
    return (
        <div className="space-y-6">
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end">
                                                        <TourDetailDialog tour={tour} onSave={addOrUpdateTour} mode="view">
                                                         ...
                                                         <DropdownMenuSeparator />
                                                         <DropdownMenuItem onSelect={() => handleOpenFahrbefehl(tour)}>Fahrbefehl erstellen</DropdownMenuItem>
            ...
            {selectedTourForFahrbefehl && (
                <FahrbefehlDialog tour={selectedTourForFahrbefehl} onGenerate={setFahrbefehlHtml} />
            )}
            
            <ReportDisplayDialog
                isOpen={isFahrbefehlOpen}
                onOpenChange={setIsFahrbefehlOpen}
                htmlContent={fahrbefehlHtml}
                onPrint={handlePrintFahrbefehl}
                title={`Fahrbefehl für Tour ${selectedTourForFahrbefehl?.tourNumber}`}
            />
